...
async def read_item(request: LinkRequest, dep: CamoufoxDep) -> LinkResponse:
    ...
    try:
        ...
        if is_post:
            ...
            cookie_header = "; ".join([f"{c['name']}={c['value']}" for c in request.cookies]) if request.cookies else None
            ...
            response_headers = dict(api_resp.headers)
